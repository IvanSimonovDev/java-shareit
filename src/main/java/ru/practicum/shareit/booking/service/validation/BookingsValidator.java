...
public class BookingsValidator {
    ...
    private void validateBookingExists(long bookingId) {
        ...
        bookingsRepository.findById(bookingId).orElseThrow(() -> new NotFoundException(excMessage));
    }

    private void validateUserOwnsItemFromBooking(long bookingId, long userId) {
        long bookingOwnerId = bookingsRepository.findById(bookingId).get().getItem().getOwner().getId();
